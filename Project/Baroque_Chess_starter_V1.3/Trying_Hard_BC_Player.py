            ...
            if curr_piece == 0:
                ...
                '''if curr_res:
                    for each_child in curr_res:
                        # new_who = 1 - whose_turn
                        # new_child = MY_BC_STATE(old_board=each_child, whose_move=new_who)
                        each_child.__class__ = MY_BC_STATE
        ...
        for multi in range(8):
            ...
            # only check leaper and king capture case
                ...
                if regular or has_leapcap:
                    ...
                    new_board[row][col] = 0
                    new_board[new_row][new_col] = 8 + whose_turn
                    new_state = BC.BC_state(new_board)
                    res[new_state] = ((row, col), (new_row, new_col))
                    # res.append(new_board)
...
def searcher(current, depth, time_limit, alpha_beta):
    ...
    best_eval, move = minimax_a_b_search(current, -math.inf, math.inf, depth, 0, who, time_limit, alpha_beta)
    new_board = cp.deepcopy(current.board)
    if move is not None:
        old_position = move[0]
        new_position = move[1]
        p = new_board[old_position[0]][old_position[1]]
        new_board[old_position[0]][old_position[1]] = 0
        new_board[new_position[0]][new_position[1]] = p
...
def minimax_a_b_search(current, a, b, depth, d, white, time_limit, a_b):
    ...
    if depth == d or len(children) == 0:  # base case
        eval_count += 1
        current.__class__ = MY_BC_STATE
        return current.static_eval(rule=1), new_move
    ...
    if white:  # maximizer
        ...
        for child in children:
            ...
            if evalu < min_eval:  # update evaluation and returned state
                min_eval = evalu
                # print("type of chlidren = {}".format(type(children[0])))
                new_move = children[child]
                if time.time() >= time_limit:
                    break
            if a_b:
...
def board_hash_value(board):
    total_hash = 0
    # print("hash_table_shape = {}".format(ZOBRIST_TABLE.shape))
    for i in range(Y_BOARD):
        for j in range(X_BOARD):
            piece = board[i][j]
            ...
            if piece != 0:
                ...
                # print("hash_piece = {}".format(hash_piece))
                total_hash ^= hash_piece
    # print("total_hash = {}".format(total_hash))
    return total_hash
